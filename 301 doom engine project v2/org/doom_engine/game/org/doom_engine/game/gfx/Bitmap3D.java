...
public class Bitmap3D extends Bitmap {
		...
		for (int y = -1; y <= level.height; y++) {
			for (int x = -1; x <= level.width; x++) {
				...
				for (int i = 0; i < c.sprites.size(); i++) {
					...
					game.enemy1.renderEnemy(this);
					game.fire.renderFire(this);
	...
	public void renderWall(double x0, double y0, double x1, double y1, int tex, int col) {
		...
		for (int x = xp0; x < xp1; x++) {
			...
			for (int y = yp0; y < yp1; y++) {
				...
				pixels[x + y * width] = Textures.textures.pixels[(xTex + tex * 16)
						+ (yTex + 0) * Textures.textures.width] * col;
			}
		}
	}
	...
	public void renderFloor(Level level) {
		for (int y = 0; y < height; y++) {
			...
			for (int x = 0; x < width; x++) {
				...
				pixels[x + y * width] = Textures.textures.pixels[((xPix & 15) + tex * 16)
						| (15 - (yPix & 15) + 16) * Textures.textures.width] * col;
			}
		}
	}
	...
	public void renderFog(int shader) {
		for (int i = 0; i < depthBuffer.length; i++) {
			...
			int brightness = (int) (255 - (depthBuffer[i] * 1 * (t * t * 3 + 1.2)));
